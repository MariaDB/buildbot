    ...
    if filters["dt"]:
        ...
        for dt_format in ("%Y-%m-%d", "%Y-%m-%d %H:%M:%S"):
            ...
            # Include the date in filtering if the passes the try/except block
                ...
                if test_failure_filters is not None:
                    test_failure_filters = test_failure_filters.filter(
                        Q(test_run_id__dt__gte=formatted_date)
                    )
                else:
                    test_failure_filters = TestFailure.objects.filter(
                        Q(test_run_id__dt__gte=formatted_date)
                    )
