...
collect_logs() {
  ...
  if (( exit_code != 0 )); then
    ...
    # Create log files (in original dir)
    : > "$systemd_log" 2>/dev/null || true
    : > "$pam_log" 2>/dev/null || true
    : > "$selinux_log" 2>/dev/null || true
    ...
    if [ -f /var/log/secure ]; then
      echo "-- grep -i pam /var/log/secure" >> "$pam_log" 2>/dev/null
      sudo grep -i pam /var/log/secure 2>&1 | sudo tee -a "$pam_log" >/dev/null
    else
      echo "/var/log/secure not present." >> "$pam_log" 2>/dev/null
    fi

    if [ -f /var/log/auth.log ]; then
      echo "-- grep -i pam /var/log/auth.log" >> "$pam_log" 2>/dev/null
      sudo grep -i pam /var/log/auth.log 2>&1 | sudo tee -a "$pam_log" >/dev/null
    else
      echo "/var/log/auth.log not present." >> "$pam_log" 2>/dev/null
    fi
  fi
